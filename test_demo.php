...
$lang = (isset($_GET['lang']) && preg_match('/^(en|fr|de|es|ja|it|pt|ar)$/', $_GET['lang'])) ? $_GET['lang'] : die('Error: Invalid language');
...
if (!isset($_GET['ext_id']) || intval($_GET['ext_id']) <= 0 ) {
	echo 'Error: Missing customer account in the URL';
	exit;
}

$external_id = $_GET['ext_id'];

#============== CLEAN TEMPORARY AND OLD FILES
shell_exec("rm -f audios/*.transcript.wav watson/* 2>/dev/null");
...
# one time charges
$result_charges = mysqli_query($db_conn, 'SELECT * FROM charges');

if (mysqli_num_rows($result_charges) <= 0) {
	echo 'Error:  No charges found in the database.';
	exit;
} else {
	while ($rec_table_datas = mysqli_fetch_row($result_charges)) {
...
# account details
$result_account = mysqli_query($db_conn, 'SELECT * FROM accounts WHERE external_id = "'.$_GET['ext_id'].'"');

if (mysqli_num_rows($result_account) <= 0) {
	echo 'Error:  No account detail found in the database.';
	exit;
} else {
	$rec_account = mysqli_fetch_row($result_account);
}


# services
$result_services = mysqli_query($db_conn, 'SELECT * FROM services WHERE account_no = '.$rec_account[0]);

if (mysqli_num_rows($result_services) <= 0) {
	echo 'Error:  No services found for the account in the database.'.PHP_EOL;
	exit;
} else {
	while ($rec_table_datas = mysqli_fetch_row($result_services)) {
		// array_push($rec_usages, $rec_table_datas[0], $rec_table_datas);
...
$result_bill_plans_discounts = mysqli_query($db_conn, <<<EOD
...
if (mysqli_num_rows($result_bill_plans_discounts) <= 0) {
	echo 'Error:  No bill plan X discount combinations found in the database.';
	exit;
} else {
	while ($rec_table_datas = mysqli_fetch_row($result_bill_plans_discounts)) {
...
$result_cust_avg_usage = mysqli_query($db_conn, <<<EOD
...
if (mysqli_num_rows($result_cust_avg_usage) <= 0) {
	echo 'Error: Unable to find montly average usage of the customer.';
	exit;
} else {
	while ($rec_table_datas = mysqli_fetch_row($result_cust_avg_usage)) {
...
$result_invoices = mysqli_query($db_conn, 'select * from invoice where account_no = '.$rec_account[0].' order by statement_date');
$num_invoices = mysqli_num_rows($result_invoices);
if ($num_invoices > 0) {
	while ($rec_table_datas = mysqli_fetch_row($result_invoices)) {
		array_push($rec_invoices, $rec_table_datas);
		
		if ($rec_table_datas[8] == '' or $rec_table_datas[8] = '0000-00-00') {
...
$tmp_subscr_no = 0;
$result_invoice_details = mysqli_query($db_conn, <<<EOD
...
if (mysqli_num_rows($result_invoice_details) > 0) {
	while ($rec_table_datas = mysqli_fetch_row($result_invoice_details)) {
...
if (isset($_GET['stage']) == 2) echo '<table><tr><td>Transcript</td><td>Audio</td></tr>';
foreach ($audio_transcript AS $trans_id => $transcript) {
	// if (!LOCAL) 
	{
		if ($trans_id==1) {
...
if (isset($_GET['stage']) == 2) echo '</table>';
if (isset($_GET['stage']) == 2) print_r($audio_transcript);

}

$audio_transcript = import_data('audio_transcript');
// print_r($audio_transcript);exit;
